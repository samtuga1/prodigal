...
// This class is responsible for capturing audio and processing frequency spectrum
class VoiceApi {
  ...
  Future<void> startRecording(void Function(List<({FrequencySpectrum spectrum, double value})> data) onData) async {
    await _isInitialized.future;

    // Configuring audio session for recording
    ...
    // Start capturing audio and process frequency spectrum
    _flutterAudioCapture.start(
      (data) {
        ...
        // Map the frequency data to the defined bands
        List<({FrequencySpectrum spectrum, double value})> frequencyValues = frequencies.map((e) {
          final min = fft.indexOfFrequency(e.min.toDouble(), 44000);
          final max = fft.indexOfFrequency(e.max.toDouble(), 44000);

          return (
            spectrum: e,
            value: freqList.sublist(min.floor(), max.ceil()).reduce((a, b) => a + b),
          );
        }).toList();

        // Pass processed data to the callback function
