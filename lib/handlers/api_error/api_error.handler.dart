...
class ApiErrorUtils {
          ...
          switch (error.type) {
            ...
            case DioExceptionType.badResponse:
              switch (error.response!.statusCode) {
                case 400:
                  httpError = BadRequest(message: error.response!.data['message'] as String);
                case 401:
                  httpError = UnauthorisedRequest(message: error.response!.data['message'] as String);
                case 403:
  ...
  static String getErrorMessage(ApiError networkExceptions) => switch (networkExceptions) {
        RequestCancelled() => 'Request Cancelled',
        UnauthorisedRequest(message: final String message) => message,
        BadRequest(message: final String message) => message,
        NotFound(message: final String message) => message.isEmpty ? 'Not found, please try again' : message,
        ...
        DefaultError(errorCode: final int responseCode) => 'Unexpected error $responseCode occurred, please try again',
